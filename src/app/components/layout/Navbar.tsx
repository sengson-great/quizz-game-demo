...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b"
      style={{ background: 'rgba(255,245,245,0.85)', backdropFilter: 'blur(24px) saturate(1.4)', borderColor: 'rgba(0,0,0,0.06)' }}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/dashboard" className="flex items-center gap-2.5 group">
            ...
            <span style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700 }}
              className="text-[#1A1A2E] text-lg tracking-wide">
                ...
                return (
                  <Link key={to} to={to}
                    className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm transition-all duration-200 ${
                      active
                        ? 'text-[#E84C6A]'
                        : 'text-slate-500 hover:text-[#1A1A2E] hover:bg-black/[0.03]'
                    }`}
                    style={active ? { background: 'rgba(232,76,106,0.08)', border: '1px solid rgba(232,76,106,0.15)' } : {}}>
              ...
              return (
                <Link key={to} to={to}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs whitespace-nowrap transition-all ${
                    active ? 'bg-[#E84C6A]/10 text-[#E84C6A]' : 'text-slate-500 hover:text-[#1A1A2E]'
                  }`}>
