...
function calculatePoints(difficulty, timeRemaining) {
...
export function GameProvider({ children }) {
    ...
    const lobbyTimerRef = React.useRef(null);
    ...
    const initGame = useCallback((mode, categoryIds) => {
    ...
    const answerQuestion = useCallback((answerId, timeRemaining) => {
        if (!gameState)
    ...
    const useLifeline = useCallback((type) => {
        if (!gameState)
            return null;
        if (gameState.lifelines[type])
            return null;
        const question = gameState.questions[gameState.currentQuestionIndex];
        const correctAnswer = question.answers.find(a => a.isCorrect);
        ...
        if (type === 'audience') {
            ...
            const data = { [correctAnswer.id]: correctVotes };
    ...
    const simulateOpponentAnswer = useCallback((questionIndex) => {
        setGameState(prev => {
            if (!prev)
                return prev;
            ...
            prev.opponents.forEach(opponent => {
                ...
                const t = setTimeout(() => {
                    setGameState(inner => {
                        if (!inner || inner.currentQuestionIndex !== questionIndex)
                            return inner;
                        ...
                        return {
                            ...inner,
                            opponents: inner.opponents.map(o => o.id === opponent.id ? { ...o, score: o.score + points, answered: true } : o),
                        };
                    });
                }, delay);
    ...
    const createSmallRoom = useCallback((categoryIds) => {
        const questions = buildGameQuestions(categoryIds);
    ...
    const createRandomSmallRoom = useCallback((categoryIds) => {
        const questions = buildGameQuestions(categoryIds);
        const roomCode = generateRoomCode();
    ...
    const joinSmallRoom = useCallback((roomCode) => {
        setGameState(prev => {
            if (!prev || prev.mode !== 'Room' || prev.status !== 'matchmaking')
                return prev;
            if (!prev.is_joinable)
                return prev;
            if (prev.lobbyPlayers?.some(p => p.id === currentUser.id))
                return prev;
            const maxPlayers = 5;
            if ((prev.lobbyPlayers?.length || 0) >= maxPlayers)
                return prev;
            const newPlayers = [...prev.lobbyPlayers, { id: currentUser.id, username: currentUser.username, avatar: currentUser.avatar, joinType: 'Code' }];
            return {
                ...prev,
    ...
    const startSmallRoomGame = useCallback(() => {
        setGameState(prev => {
            if (!prev || prev.mode !== 'Room' || prev.status !== 'matchmaking')
                return prev;
            if (!prev.lobbyPlayers || prev.lobbyPlayers.length < 3)
                return prev; // Min 3 players
    ...
    const extendLobbyTimer = useCallback(() => {
        setGameState(prev => {
            if (!prev || prev.status !== 'matchmaking')
                return prev;
            if (prev.lobbyStatus !== 'waiting')
                return prev;
            return {
                ...prev,
                lobbyCountdown: 60,
    ...
    const addAIPlayers = useCallback((count) => {
        setGameState(prev => {
            if (!prev || prev.mode !== 'Room' || prev.status !== 'matchmaking')
                return prev;
            const maxPlayers = 5;
    ...
    const createPrivate1v1 = useCallback((categoryIds) => {
        const questions = buildGameQuestions(categoryIds);
        const roomCode = generateRoomCode();
        const expiresAt = Date.now() + 60 * 1000; // 60 seconds
        setGameState({
    ...
    const joinPrivate1v1 = useCallback((roomCode) => {
        setGameState(prev => {
            if (!prev || prev.mode !== '1v1' || prev.status !== 'matchmaking')
                return prev;
            if (!prev.is_joinable)
                return prev;
            if (prev.lobbyPlayers?.some(p => p.id === currentUser.id))
                return prev;
            if ((prev.lobbyPlayers?.length || 0) >= 2)
                return prev;
            return {
                ...prev,
                lobbyPlayers: [...prev.lobbyPlayers, { id: currentUser.id, username: currentUser.username, avatar: currentUser.avatar, joinType: 'Code' }],
                is_joinable: false,
            };
        });
    }, [currentUser]);
    // Random Match: Search for human, fallback to AI "System Random"
    const startRanked1v1 = useCallback((categoryIds) => {
        const questions = buildGameQuestions(categoryIds);
        const roomCode = generateRoomCode();
        setGameState({
            mode: '1v1',
    ...
    const switchToRandom = useCallback(() => {
        setGameState(prev => {
            if (!prev || prev.mode !== '1v1' || !prev.isPrivate)
                return prev;
            return {
                ...prev,
                isPrivate: false,
    ...
    const startBattle = useCallback(() => {
        setGameState(prev => {
            if (!prev || prev.mode !== '1v1' || prev.status !== 'matchmaking')
                return prev;
            if (!prev.lobbyPlayers || prev.lobbyPlayers.length < 2)
                return prev;
            const opponents = prev.lobbyPlayers
                .filter(player => player.id !== currentUser.id)
                .map(player => ({
