...
export function getQuestionsByDifficulty(difficulty: Question['difficulty'], categoryIds: string[], count: number): Question[] {
  const pool = QUESTIONS.filter(q => q.difficulty === difficulty && categoryIds.includes(q.categoryId));
  const shuffled = [...pool].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, count);
}

export function buildGameQuestions(categoryIds: string[]): Question[] {
