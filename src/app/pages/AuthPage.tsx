  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative"
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/3 w-[500px] h-[500px] rounded-full opacity-[0.3]"
          style={{ background: 'radial-gradient(circle, #FCE4EC, transparent)', filter: 'blur(100px)' }} />
      </div>
      ...
      >
        ...
        <div className="rounded-2xl overflow-hidden"
          style={{ background: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(20px)', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.06)' }}>
          {/* Tabs */}
            ...
            {(['login', 'register'] as const).map(t => (
              <button key={t} onClick={() => { setTab(t); setError(''); }}
                className={`flex-1 py-4 text-sm transition-all capitalize ${
                  tab === t ? 'text-[#E84C6A] border-b-2 border-[#E84C6A]' : 'text-slate-400 hover:text-slate-600'
                }`}
                style={tab === t ? { background: 'rgba(232,76,106,0.04)' } : {}}>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <AnimatePresence>
                  {error && (
                    <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                      className="flex items-center gap-2 p-3 rounded-xl text-red-600 text-sm"
                ...
                <button type="submit" disabled={loading}
                  ...
                  style={{ background: loading ? '#C43555' : 'linear-gradient(135deg, #E84C6A, #D43B59)', boxShadow: '0 4px 15px rgba(232,76,106,0.3)' }}>
                  {loading ? (
