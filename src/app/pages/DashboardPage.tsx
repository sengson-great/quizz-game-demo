      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map(({ label, value, icon: Icon, color, bgStyle }, i) => (
          <motion.div key={label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
            className="rounded-2xl p-5" style={{ ...bgStyle, backdropFilter: 'blur(20px)' }}>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {gameModes.map(({ icon, title, desc, badge, badgeStyle, action }, i) => (
              <motion.button key={title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }}
                whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} onClick={action}
                className="group text-left rounded-2xl p-5 cursor-pointer transition-all" style={CARD}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          <button onClick={() => navigate('/leaderboard')}
            className="text-[#E84C6A] hover:text-[#D43B59] text-sm flex items-center gap-1 transition-colors">
            View All <ChevronRight className="w-3 h-3" />
          </button>
        </div>
        <div className="divide-y" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
          {MOCK_GAME_HISTORY.map((game, i) => (
            <motion.div key={game.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + i * 0.05 }}
          ...
          {MOCK_GAME_HISTORY.length === 0 && (
            <div className="py-12 text-center text-slate-400">No games yet. Start playing!</div>
          )}
        </div>
      </motion.div>
    </div>
