...
const BAR_COLORS = ['#E84C6A', '#F472B6', '#06b6d4', '#34d399', '#fbbf24', '#8b5cf6'];
...
function CategoryBar({ subject, value, icon, color, index }: { subject: string; value: number; icon: string; color: string; index: number }) {
  return (
    ...
    >
      ...
      <span className="text-[#6B7280] text-xs w-16 flex-shrink-0">{subject}</span>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-[#1A1A2E]" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700 }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map(({ label, value, icon: Icon, color, bgStyle }, i) => (
          <motion.div key={label}
            ...
            style={{ ...bgStyle, background: '#FFFFFF', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.04)' }}>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {gameModes.map(({ icon, title, desc, badge, badgeStyle, action }, i) => (
              <motion.button key={title}
                ...
                <ChevronRight className="w-4 h-4 text-[#9CA3AF] group-hover:text-[#E84C6A] mt-3 transition-colors" />
              </motion.button>
            ))}
          </div>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
          ...
          <button onClick={() => navigate('/leaderboard')}
            className="text-[#E84C6A] hover:text-[#D63B5C] text-sm flex items-center gap-1 transition-colors">
        ...
        <div className="divide-y" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
          {MOCK_GAME_HISTORY.map((game, i) => (
            <motion.div key={game.id}
              ...
              className="flex items-center justify-between px-6 py-4 hover:bg-[#FFF5F5]/50 transition-colors">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg"
                  style={{ background: 'rgba(232,76,106,0.04)' }}>
              ...
              <div className="text-right">
                ...
                <p className="text-[#9CA3AF] text-xs">{game.correctAnswers}/{game.totalQuestions} correct</p>
              </div>
            </motion.div>
          ))}
          {MOCK_GAME_HISTORY.length === 0 && (
            <div className="py-12 text-center text-[#9CA3AF]">No games yet. Start playing!</div>
          )}
        </div>
      </motion.div>
    </div>
