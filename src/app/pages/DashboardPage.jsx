...
export default function DashboardPage() {
    ...
    useEffect(() => { if (!currentUser)
        navigate('/auth'); }, [currentUser, navigate]);
    if (!currentUser)
        return null;
    const winRate = currentUser.gamesPlayed > 0 ? Math.round((currentUser.wins / currentUser.gamesPlayed) * 100) : 0;
    const stats = [
        { label: 'Total Score', value: currentUser.totalScore.toLocaleString(), icon: Star, color: 'text-amber-500', bgStyle: { background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.15)' } },
        { label: 'Games Played', value: currentUser.gamesPlayed, icon: Target, color: 'text-[#E84C6A]', bgStyle: { background: 'rgba(232,76,106,0.06)', border: '1px solid rgba(232,76,106,0.12)' } },
        { label: 'Win Rate', value: `${winRate}%`, icon: TrendingUp, color: 'text-emerald-500', bgStyle: { background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.12)' } },
        { label: 'Global Rank', value: currentUser.rank > 0 ? `#${currentUser.rank}` : 'Unranked', icon: Trophy, color: 'text-cyan-500', bgStyle: { background: 'rgba(6,182,212,0.06)', border: '1px solid rgba(6,182,212,0.12)' } },
    ];
    const gameModes = [
        { icon: '🎯', title: 'Solo Practice', desc: '15 questions, no pressure.', badge: 'Best for Practice', badgeStyle: { background: 'rgba(251,191,36,0.1)', color: '#d97706' }, action: () => navigate('/mode-select', { state: { preMode: 'Solo' } }) },
        { icon: '⚔️', title: '1v1 Battle', desc: 'Duel a live opponent.', badge: 'Recommended', badgeStyle: { background: 'rgba(232,76,106,0.08)', color: '#E84C6A' }, action: () => navigate('/mode-select', { state: { preMode: '1v1' } }) },
        { icon: '🏆', title: 'Room Battle', desc: '3-5 players, live scoreboard.', badge: 'Most Fun', badgeStyle: { background: 'rgba(52,211,153,0.08)', color: '#059669' }, action: () => navigate('/mode-select', { state: { preMode: 'Room' } }) },
    ];
    const modeStyle = {
    ...
    return (<div className="min-h-screen px-4 py-8 max-w-7xl mx-auto" style={{ fontFamily: 'Poppins, Inter, sans-serif' }}>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => navigate('/mode-select')} className="hidden sm:flex items-center gap-2 px-6 py-3 rounded-xl text-white text-sm transition-all hover:scale-[1.03]" style={{ background: 'linear-gradient(135deg, #E84C6A, #D43B59)', boxShadow: '0 4px 15px rgba(232,76,106,0.3)' }}>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-[#1A1A2E] text-lg" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>Quick Play</h2>
          <div className="grid sm:grid-cols-3 gap-4">
            {gameModes.map(({ icon, title, desc, badge, badgeStyle, action }, i) => (<motion.button key={title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }} whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} onClick={action} className="group text-left rounded-2xl p-5 cursor-pointer transition-all" style={CARD}>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="rounded-2xl p-5" style={CARD}>
          <h2 className="text-[#1A1A2E] text-sm mb-5" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>Category Performance</h2>
          <div className="space-y-3">
            {CATEGORY_DATA.map(({ subject, value, icon }, i) => (<CategoryBar key={subject} subject={subject} value={value} icon={icon} color={BAR_COLORS[i]} index={i}/>))}
          </div>
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="rounded-2xl overflow-hidden" style={CARD}>
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
          <h2 className="text-[#1A1A2E]" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>Recent Games</h2>
        ...
        <div className="divide-y" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
          {MOCK_GAME_HISTORY.map((game, i) => (<motion.div key={game.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + i * 0.05 }} className="flex items-center justify-between px-6 py-4 hover:bg-black/[0.01] transition-colors">
              <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                <p className="text-[#1A1A2E]" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>{game.score.toLocaleString()}</p>
                <p className="text-slate-400 text-xs">{game.correctAnswers}/{game.totalQuestions} correct</p>
              </div>
            </motion.div>))}
          {MOCK_GAME_HISTORY.length === 0 && (<div className="py-12 text-center text-slate-400">No games yet. Start playing!</div>)}
        </div>
      </motion.div>
    </div>);
}
