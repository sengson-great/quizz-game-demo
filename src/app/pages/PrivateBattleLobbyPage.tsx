...
export default function PrivateBattleLobbyPage() {
  ...
  useEffect(() => {
    if (!gameState || gameState.mode !== '1v1') { navigate('/mode-select'); return; }
    if (!currentUser) { navigate('/auth'); return; }
    if (gameState.is_ranked && !gameState.isPrivate) { setSwitchedToRandom(true); setSearchingRandom(true); }
  }, [gameState?.mode, currentUser, navigate]);

  useEffect(() => {
    if (gameStarting || opponentJoined) return;
    const interval = setInterval(() => { setCountdown(prev => { if (prev <= 1) { clearInterval(interval); resetGame(); navigate('/mode-select', { state: { preMode: '1v1' } }); return 0; } return prev - 1; }); }, 1000);
    return () => clearInterval(interval);
  }, [navigate, resetGame, gameStarting, opponentJoined]);

  useEffect(() => {
    if (switchedToRandom || searchingRandom) return;
  ...
  useEffect(() => {
    ...
    searchTimerRef.current = setTimeout(() => {
      ...
      if (isHuman) { setSimulatedOpponent({ username: opponentPool[0].username, avatar: opponentPool[0].avatar }); }
      else { setSimulatedOpponent({ username: 'System Random', avatar: '🤖', isSystem: true }); }
      setOpponentJoined(true); setSearchingRandom(false);
    }, searchTime);
  ...
  const handleStart = () => { if (!opponentJoined || !gameState) return; setGameStarting(true); setTimeout(() => navigate('/game'), 1500); };
      ...
      <div className="relative z-10 max-w-2xl mx-auto pt-12">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          <div className="inline-block px-4 py-1.5 rounded-full mb-4"
            style={{ background: switchedToRandom ? 'rgba(6,182,212,0.06)' : 'rgba(232,76,106,0.06)', border: `1px solid ${switchedToRandom ? 'rgba(6,182,212,0.15)' : 'rgba(232,76,106,0.12)'}` }}>
        ...
        {isPrivateMode && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-2xl p-6 mb-6" style={CARD}>
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={copyRoomLink}
              className="w-full py-2.5 rounded-xl flex items-center justify-center gap-2 text-sm transition-all mb-3"
              style={{ background: 'rgba(232,76,106,0.06)', border: '1px solid rgba(232,76,106,0.12)', color: '#E84C6A' }}>
        ...
        {searchingRandom && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-8 mb-6 text-center" style={CARD}>
            <div className="relative w-24 h-24 mx-auto mb-4">
              {[0, 1, 2].map(i => (
                <motion.div key={i} className="absolute inset-0 rounded-full" style={{ border: '2px solid rgba(6,182,212,0.2)' }}
                  animate={{ scale: [1, 2.5], opacity: [0.5, 0] }} transition={{ duration: 2, delay: i * 0.6, repeat: Infinity }} />
              ))}
              <div className="absolute inset-0 flex items-center justify-center">
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 3, repeat: Infinity, ease: 'linear' }}
                  className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: 'rgba(6,182,212,0.08)', border: '2px solid rgba(6,182,212,0.15)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-2xl p-6 mb-6" style={CARD}>
          ...
          <div className="space-y-3">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-4 p-4 rounded-xl"
        ...
        {isPrivateMode && !opponentJoined && !gameStarting && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-6">
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleSwitchToRandom}
              className="w-full py-4 rounded-xl text-white flex items-center justify-center gap-3 transition-all"
              style={{ background: 'linear-gradient(135deg, #dc2626, #b91c1c)', boxShadow: '0 4px 20px rgba(220,38,38,0.25)', fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>
        ...
        <AnimatePresence>
          {countdown <= 15 && !opponentJoined && !searchingRandom && (
        ...
        <AnimatePresence>
          {gameStarting && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl p-6 mb-6 text-center"
        ...
        {!gameStarting && (
          <div className="flex gap-4">
            <motion.button whileHover={{ scale: canStart ? 1.02 : 1 }} whileTap={{ scale: canStart ? 0.98 : 1 }} onClick={handleStart} disabled={!canStart}
              className={`flex-1 py-4 rounded-xl text-white flex items-center justify-center gap-2 transition-all ${canStart ? 'cursor-pointer' : 'opacity-40 cursor-not-allowed'}`}
              style={{ background: canStart ? 'linear-gradient(135deg, #E84C6A, #D43B59)' : 'rgba(0,0,0,0.06)', boxShadow: canStart ? '0 4px 20px rgba(232,76,106,0.3)' : 'none', fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>
              <Play className="w-5 h-5" />{canStart ? 'Start Battle' : 'Waiting for Opponent'}
            </motion.button>
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleCancel}
              className="py-4 rounded-xl text-slate-500 transition-all px-8"
              style={{ background: 'rgba(255,255,255,0.6)', border: '1px solid rgba(0,0,0,0.08)', fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>Cancel</motion.button>
          </div>
        )}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <div className="rounded-2xl p-6 shadow-2xl" style={MODAL_BG}>
                ...
                <div className="flex gap-3">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowSwitchConfirm(false)}
                    className="flex-1 py-3 rounded-xl text-[#1A1A2E] transition-all" style={{ background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.08)', fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>Keep Waiting</motion.button>
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={confirmSwitchToRandom}
                    className="flex-1 py-3 rounded-xl text-white transition-all" style={{ background: 'linear-gradient(135deg, #dc2626, #b91c1c)', boxShadow: '0 2px 15px rgba(220,38,38,0.25)', fontFamily: 'Poppins, sans-serif', fontWeight: 600 }}>Switch to Random</motion.button>
                </div>
              </div>
            </motion.div>
          </>
