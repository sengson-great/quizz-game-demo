...
const FEATURES = [
  { icon: Zap, title: 'Solo Practice', desc: '15 progressive questions across 3 difficulty levels. Sharpen your skills at your own pace.', color: 'text-amber-500', gradient: 'from-amber-500 to-orange-500' },
  { icon: Swords, title: '1v1 Battle', desc: 'Go head-to-head against another player with real-time score tracking.', color: 'text-[#E84C6A]', gradient: 'from-[#E84C6A] to-[#D63B5C]' },
  { icon: Users, title: 'Room Mode', desc: '3-5 players compete together. Create rooms, share codes, and battle!', color: 'text-cyan-500', gradient: 'from-cyan-500 to-teal-500' },
];
...
export default function LandingPage() {
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(145deg, #FFF5F5 0%, #FDE8EC 40%, #FCE4EC 70%, #FFF0F3 100%)', fontFamily: 'Poppins, Inter, sans-serif' }}>
      {/* Ambient BG */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-32 left-1/3 w-[700px] h-[700px] rounded-full opacity-[0.2]"
          style={{ background: 'radial-gradient(circle, #E84C6A, transparent)', filter: 'blur(120px)' }} />
        <div className="absolute top-1/3 right-0 w-[500px] h-[500px] rounded-full opacity-[0.15]"
          style={{ background: 'radial-gradient(circle, #F472B6, transparent)', filter: 'blur(120px)' }} />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full opacity-[0.1]"
          style={{ background: 'radial-gradient(circle, #FCA5A5, transparent)', filter: 'blur(100px)' }} />
      </div>
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 py-5 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          <Link to="/leaderboard" className="text-[#6B7280] hover:text-[#1A1A2E] text-sm transition-colors">Leaderboard</Link>
          <Link to="/auth" className="px-5 py-2.5 rounded-xl text-white text-sm transition-all hover:shadow-lg hover:scale-[1.02]"
            style={{ background: 'linear-gradient(135deg, #E84C6A, #D63B5C)', boxShadow: '0 2px 15px rgba(232,76,106,0.35)' }}>
            Get Started
          </Link>
        </div>
      </nav>
      ...
      <section className="relative z-10 text-center px-4 pt-16 pb-24 max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm mb-8"
            style={{ background: 'rgba(232,76,106,0.08)', border: '1px solid rgba(232,76,106,0.15)', color: '#E84C6A' }}>
          ...
          <h1 className="text-[#1A1A2E] mb-6" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(2.5rem, 7vw, 4.5rem)', lineHeight: 1.1 }}>
            Challenge Your<br />
            <span style={{ background: 'linear-gradient(135deg, #E84C6A, #F472B6, #D63B5C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Knowledge
            </span>
          </h1>
          <p className="text-[#6B7280] max-w-2xl mx-auto mb-10 leading-relaxed" style={{ fontSize: 'clamp(1rem, 2vw, 1.125rem)' }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/auth"
              ...
              style={{ background: 'linear-gradient(135deg, #E84C6A, #D63B5C)', boxShadow: '0 4px 25px rgba(232,76,106,0.4)' }}>
              <Brain className="w-5 h-5" />
              Play Now
              <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Link>
            <Link to="/leaderboard"
              className="flex items-center gap-2 px-8 py-4 rounded-2xl text-[#1A1A2E] transition-all hover:bg-white/60"
        ...
        >
          {STATS.map(({ value, label, icon: Icon }) => (
            <div key={label} className="rounded-2xl p-5 text-center"
              style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(20px)', border: '1px solid rgba(0,0,0,0.04)', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
      ...
      <section className="relative z-10 px-4 py-20 max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
          <h2 className="text-center text-[#1A1A2E] mb-3" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700 }}>
            Choose Your Battle
          </h2>
          <p className="text-center text-[#6B7280] mb-12">Three game modes to match every mood</p>
          <div className="grid md:grid-cols-3 gap-6">
            {FEATURES.map(({ icon: Icon, title, desc, color, gradient }, i) => (
              <motion.div key={title}
                ...
                style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(20px)', border: '1px solid rgba(0,0,0,0.04)', boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}>
      ...
      <section className="relative z-10 px-4 py-20 max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
          ...
          <p className="text-center text-[#6B7280] mb-12">Difficulty scales from Easy to Medium to Hard</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {CATEGORIES.map(({ icon, name }, i) => (
              <motion.div key={name}
                ...
                <p className="text-[#6B7280] text-xs text-center">{name}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
      ...
      <section className="relative z-10 px-4 py-20 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
          <h2 className="text-center text-[#1A1A2E] mb-12" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700 }}>
            How It Works
          </h2>
      ...
      <section className="relative z-10 px-4 py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <div className="max-w-2xl mx-auto rounded-3xl p-12"
            style={{ background: 'rgba(232,76,106,0.06)', backdropFilter: 'blur(20px)', border: '1px solid rgba(232,76,106,0.12)', boxShadow: '0 4px 40px rgba(232,76,106,0.08)' }}>
            <Shield className="w-12 h-12 text-[#E84C6A] mx-auto mb-4" />
            <h2 className="text-[#1A1A2E] mb-4" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700 }}>
              Ready to Test Yourself?
            </h2>
            <p className="text-[#6B7280] mb-8">Join thousands of players competing in real-time quiz battles.</p>
            <Link to="/auth"
              className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl text-white transition-all hover:scale-[1.03]"
              style={{ background: 'linear-gradient(135deg, #E84C6A, #D63B5C)', boxShadow: '0 4px 30px rgba(232,76,106,0.4)' }}>
              Start Playing Free
      ...
      <footer className="relative z-10 text-center py-8 text-[#9CA3AF] text-sm">
        <p>2026 QuizBlitz</p>
      </footer>
    </div>
  );
