...
export default function ResultsPage() {
  ...
  const getGrade = () => {
    if (accuracy >= 90) return { grade: 'S', label: 'Perfect!' };
    if (accuracy >= 80) return { grade: 'A', label: 'Excellent!' };
    if (accuracy >= 70) return { grade: 'B', label: 'Great job!' };
    if (accuracy >= 60) return { grade: 'C', label: 'Good effort!' };
                ...
                {gameState.answers.map((answer, i) => {
                  ...
                  return (
                    <div key={answer.questionId}
                      ...
                      style={{ background: answer.isCorrect ? 'rgba(52,211,153,0.06)' : 'rgba(239,68,68,0.06)' }}>
                      {answer.isCorrect
