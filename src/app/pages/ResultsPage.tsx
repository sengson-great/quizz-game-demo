...
export default function ResultsPage() {
  ...
  useEffect(() => { if (!gameState || !currentUser) { navigate('/dashboard'); return; } }, []);
  useEffect(() => {
    if (!gameState) return;
    const target = gameState.playerScore;
    let current = 0;
    const step = Math.max(1, Math.floor(target / 60));
    const interval = setInterval(() => { current = Math.min(current + step, target); setAnimScore(current); if (current >= target) clearInterval(interval); }, 20);
  ...
  const getGrade = () => { if (accuracy >= 90) return { grade: 'S', label: 'Perfect!' }; if (accuracy >= 80) return { grade: 'A', label: 'Excellent!' }; if (accuracy >= 70) return { grade: 'B', label: 'Great job!' }; if (accuracy >= 60) return { grade: 'C', label: 'Good effort!' }; return { grade: 'D', label: 'Keep practicing!' }; };
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-3xl mx-auto" style={{ fontFamily: 'Poppins, Inter, sans-serif' }}>
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
          {Array.from({ length: 20 }, (_, i) => (
            <motion.div key={i} className="absolute w-3 h-3 rounded-sm"
              style={{ left: `${Math.random() * 100}%`, background: ['#E84C6A', '#F472B6', '#06b6d4', '#34d399'][i % 4], top: '-20px' }}
      ...
      {gameState.mode !== 'Solo' && allScores.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="rounded-2xl p-5 mb-6 relative z-10" style={CARD}>
          ...
          <div className="space-y-3">
            {allScores.map((player, i) => (
              <motion.div key={player.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.1 }}
                className="flex items-center gap-3 p-3 rounded-xl"
                style={{ background: player.isPlayer ? 'rgba(232,76,106,0.06)' : 'rgba(0,0,0,0.02)', border: player.isPlayer ? '1px solid rgba(232,76,106,0.12)' : '1px solid transparent' }}>
      ...
      {categoryBreakdown.length > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="rounded-2xl p-5 mb-6 relative z-10" style={CARD}>
              ...
              return (
                <div key={id}>
                  ...
                  <div className="h-2 rounded-full overflow-hidden" style={{ background: 'rgba(0,0,0,0.06)' }}>
                    <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, delay: 0.6 }} className="h-full rounded-full"
                      style={{ background: pct === 100 ? '#34d399' : pct >= 60 ? '#fbbf24' : '#f87171' }} />
                  </div>
                </div>
              );
            })}
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
              <div className="px-5 pb-4 space-y-2 pt-4" style={{ borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                {gameState.answers.map((answer, i) => {
                  const q = gameState.questions[i]; if (!q) return null;
                  const cat = CATEGORIES.find(c => c.id === q.categoryId);
                  return (
                    <div key={answer.questionId} className="flex items-start gap-3 p-3 rounded-xl text-sm"
                      style={{ background: answer.isCorrect ? 'rgba(52,211,153,0.04)' : 'rgba(239,68,68,0.04)' }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="flex gap-3 relative z-10">
        <button onClick={() => { resetGame(); navigate('/dashboard'); }}
          className="flex-1 flex items-center justify-center gap-2 py-4 rounded-xl text-slate-600 hover:bg-white/60 transition-colors text-sm"
