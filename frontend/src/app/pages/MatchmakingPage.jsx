...
export default function MatchmakingPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center px-4" style={{ background: LIGHT_BG, fontFamily: 'inherit' }}>
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full opacity-[0.25]" style={{ background: 'radial-gradient(circle, rgba(250, 204, 21, 0.05), transparent)', filter: 'blur(120px)' }}/>
            </div>
                ...
                <AnimatePresence mode="wait">
                    {(stage === 'searching' || stage === 'filling') && (
                        <motion.div key="searching" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                            <div className="relative w-32 h-32 mx-auto mb-8">
                                {[0, 1, 2].map(i => (
                                    <motion.div key={i} className="absolute inset-0 rounded-full" style={{ border: `2px solid ${isRanked1v1 ? 'rgba(6,182,212,0.2)' : 'rgba(250,204,21,0.2)'}` }} animate={{ scale: [1, 2.5], opacity: [0.5, 0] }} transition={{ duration: 2, delay: i * 0.6, repeat: Infinity }}/>
                                ))}
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(20px)', border: `1px solid ${isRanked1v1 ? 'rgba(6,182,212,0.15)' : 'rgba(250,204,21,0.15)'}`, boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
                                ...
                                <div className="rounded-2xl p-6 mb-4" style={CARD}>
                                    ...
                                    <div className="mt-3 space-y-2">
                                        {gameState.lobbyPlayers?.slice(0, joinedPlayers).map((player) => (
                                            <motion.div key={player.id} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring', stiffness: 300 }} className="flex items-center gap-2 justify-center text-sm">
                                                ...
                                                <span className={player.id === currentUser.id ? 'text-[#FACC15]' : 'text-[#1A1A2E]'}>
                                    ...
                                    {gameState.isHost && joinedPlayers >= 2 && joinedPlayers < totalPlayersInRoom && (
                                        <motion.button 
                                            ...
                                            style={{ background: 'linear-gradient(135deg, #FACC15, #065F46)', boxShadow: '0 4px 15px rgba(250,204,21,0.25)', fontFamily: 'inherit', fontWeight: 600 }}
                                        >
                                    ...
                                    {gameState.lobbyInviteCode && (
                                        <motion.div
                                            ...
                                            style={{ background: 'rgba(250,204,21,0.06)', border: '1px dashed rgba(250,204,21,0.3)' }}
                                        >
                                            <p className="text-xs text-slate-400 mb-1.5" style={{ fontFamily: 'inherit' }}>Invite friends with code</p>
                                            <div className="flex items-center justify-center gap-3">
                                                <span
                                                    className="tracking-[0.3em] text-[#FACC15] font-mono"
                                                ...
                                                <motion.button
                                                    ...
                                                    style={{
                                                        background: copied ? 'rgba(52,211,153,0.12)' : 'rgba(250,204,21,0.10)',
                                                        border: copied ? '1px solid rgba(52,211,153,0.3)' : '1px solid rgba(250,204,21,0.2)',
                                                        color: copied ? '#34d399' : '#FACC15',
                                                    }}
                                                    title="Copy code"
                                                >
                                            ...
                                            <p className="text-xs text-slate-400 mt-1.5" style={{ fontFamily: 'inherit' }}>Friends can join from Mode Select → Room → Join</p>
                                        </motion.div>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="flex items-center justify-center gap-4 mt-6 flex-wrap">
                                {allPlayers.map((p, i) => (
                                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="flex flex-col items-center gap-2">
                                        <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl" style={{ background: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(20px)', border: p.isYou ? '2px solid #FACC15' : '1px solid rgba(0,0,0,0.06)', boxShadow: p.isYou ? '0 0 20px rgba(250,204,21,0.15)' : '0 2px 8px rgba(0,0,0,0.04)' }}>
                                            {p.avatar}
                                        </div>
                    ...
                    {stage === 'ready' && (
                        <motion.div key="ready" initial={{ opacity: 0, scale: 1.2 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
                            <motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 0.5, repeat: countdown > 0 ? Infinity : 0 }} className="text-[8rem] mx-auto mb-4 flex items-center justify-center" style={{ fontFamily: 'inherit', fontWeight: 800, color: countdown > 1 ? '#FACC15' : countdown === 1 ? '#d97706' : '#FACC15', filter: `drop-shadow(0 0 30px currentColor)` }}>
