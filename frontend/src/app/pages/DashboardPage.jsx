...
export default function DashboardPage() {
    ...
    const stats = [
        { label: t('totalScore'), value: currentUser.totalScore.toLocaleString(), icon: Star, color: 'text-amber-500', bgStyle: { background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.15)' } },
        { label: t('statsGames'), value: currentUser.gamesPlayed, icon: Target, color: 'text-[#E84C6A]', bgStyle: { background: 'rgba(232,76,106,0.06)', border: '1px solid rgba(232,76,106,0.12)' } },
        { label: t('winRate'), value: `${winRate}%`, icon: TrendingUp, color: 'text-emerald-500', bgStyle: { background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.12)' } },
        { label: t('rank'), value: currentUser.rank > 0 ? `#${currentUser.rank}` : t('noData').split('.')[0], icon: Trophy, color: 'text-cyan-500', bgStyle: { background: 'rgba(6,182,212,0.06)', border: '1px solid rgba(6,182,212,0.12)' } },
    ];
    ...
    const CATEGORY_DATA = [
        { id: 'science', subject: t('catScience'), value: 78, icon: '🔬' },
        { id: 'history', subject: t('catHistory'), value: 62, icon: '📜' },
        { id: 'technology', subject: t('catTechnology'), value: 85, icon: '💻' },
        { id: 'geography', subject: t('catGeography'), value: 55, icon: '🌍' },
        { id: 'sports', subject: t('catSports'), value: 70, icon: '⚽' },
        { id: 'arts', subject: t('catArts'), value: 48, icon: '🎨' },
    ];
        ...
        <div className="divide-y" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
          {MOCK_GAME_HISTORY.map((game, i) => (<motion.div key={game.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + i * 0.05 }} className="flex items-center justify-between px-6 py-4 hover:bg-black/[0.01] transition-colors">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {game.rank && <span className="text-xs text-amber-500">#{game.rank} {t('rank')}</span>}
                  </div>
              ...
              <div className="text-right">
                ...
                <p className="text-slate-400 text-xs">{game.correctAnswers}/{game.totalQuestions} {t('correct').toLowerCase()}</p>
              </div>
            </motion.div>))}
          {MOCK_GAME_HISTORY.length === 0 && (<div className="py-12 text-center text-slate-400">{t('noRecentMatches')}</div>)}
        </div>
      </motion.div>
    </div>);
}
